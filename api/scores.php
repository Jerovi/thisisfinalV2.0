...
function handleScoreSubmission(array $input, int $userId): void {
    ...
    // FIX: Extract play_time correctly. Games send it inside game_data.
    $playTime = (int)($input['play_time'] ?? $gameData['play_time'] ?? 0);
    ...
    if (defined('SCORE_VALIDATION_ENABLED') && SCORE_VALIDATION_ENABLED && !validateGameSession($sessionToken, $userId, $gameId)) {
    ...
    try {
        ...
        $scoreId = Database::insert(
            ...
            [
                ...
                $sessionToken,
                (defined('ANTI_CHEAT_ENABLED') && ANTI_CHEAT_ENABLED) ? 1 : 0
            ]
        );
        ...
        if ($pointsEarned > 0) {
            ...
            // FIX: Auto-create wallet if missing (Prevents rewards not working issue)
            if (!$wallet) {
                Database::insert(
                    "INSERT INTO wallet (user_id, balance, total_earned, created_at, updated_at) VALUES (?, 0, 0, NOW(), NOW())",
                    [$userId]
                );
                $wallet = Database::fetch(
                    "SELECT id, balance FROM wallet WHERE user_id = ?",
            ...
            if ($wallet) {
                ...
                // Record transaction
                Database::insert(
                    "INSERT INTO transactions (wallet_id, type, amount, balance_after, description, reference_type, reference_id, created_at) 
                     VALUES (?, 'earn', ?, ?, ?, 'game', ?, NOW())",
                    [
                        $wallet['id'],
                        $pointsEarned,
                        $newBalance,
            ...
            // Get current balance if no points earned, creating wallet if needed to show correct 0
...
function validateScore(int $score, array $game, array $gameData, int $playTime): array {
    ...
    // Only check if playTime is actually recorded (>0)
    $minPlayTime = getMinPlayTime($game['slug']);
    if ($playTime > 0 && $playTime < $minPlayTime) {
        return ['valid' => false, 'reason' => "Play time too short ($playTime sec)"];
    }
...
function getMinPlayTime(string $gameSlug): int {
    return match($gameSlug) {
        'fruit-catch' => 10,  // Lowered for fast players
        'word-scramble' => 5,
        'quiz-master' => 10,
        'sliding-puzzle' => 5,
        'memory-match' => 10,
